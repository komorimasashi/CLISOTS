...
if __name__ == '__main__':
    ...
    if not (torch.cuda.is_available() or (getattr(torch.backends, "mps", None) is not None and torch.backends.mps.is_available())):
        use_gpu = False
    ...
    for seed in seeds:
        # wandb_name = f"{test_name}_{args.prompt}_{args.clip_fc_loss_weight}_{args.num_strokes}strokes_seed{seed}"
        wandb_name = f"{test_name}_{args.clip_fc_loss_weight}_{args.num_strokes}strokes_seed{seed}"
        if multiprocess:
    ...
    copyfile(f"{output_dir}/{list(sorted_final.keys())[0]}/best_iter.png",
            f"{output_dir}/{list(sorted_final.keys())[0]}_best.png")
    copyfile(f"{output_dir}/{list(sorted_final.keys())[0]}/best_iter.png",
            f"{abs_path}/target_images/last.png")
